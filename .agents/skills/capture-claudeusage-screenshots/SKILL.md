---
name: capture-claudeusage-screenshots
description: Automate ClaudeUsage interface screenshots with CleanShot X on macOS. Use when Codex needs to capture localized ClaudeUsage menu-bar popover screenshots, switch ClaudeUsage display/language settings, save files such as detail.claude.en@2x.png, or troubleshoot CleanShot/window-capture automation that depends on macOS Accessibility, osascript, CoreGraphics mouse movement, and the ClaudeUsage Debug app.
---

# Capture ClaudeUsage Screenshots

## Overview

Capture ClaudeUsage popover screenshots by matching the user's manual CleanShot workflow: enter CleanShot window capture mode, hover the real mouse over the ClaudeUsage popover, click, and rename the newly saved file in Downloads.
...
- Use the Debug ClaudeUsage instance when the user requests it.
...
## Configure ClaudeUsage

Set the screenshot state in ClaudeUsage settings before capture.
...
- Switch language only inside ClaudeUsage settings. Do not change language by writing `UserDefaults` or by external app-level language overrides.
- Keep the ClaudeUsage screenshot setting that prevents the popover from closing.

After changing settings, open the main ClaudeUsage popover and inspect it with Computer Use. The screenshot should show the intended language and display items before CleanShot starts.
...
```sh
scripts/capture_claudeusage_window.sh detail.claude.en@2x.png
```

The script:

1. Records the current latest CleanShot file in `~/Downloads`.
2. Finds the on-screen ClaudeUsage popover bounds with `CGWindowListCopyWindowInfo`.
...
Find the ClaudeUsage popover bounds:

```sh
swift -e 'import CoreGraphics; if let windows = CGWindowListCopyWindowInfo([.optionOnScreenOnly, .excludeDesktopElements], kCGNullWindowID) as? [[String: Any]] { for w in windows { let owner = w[kCGWindowOwnerName as String] as? String ?? ""; if owner.contains("ClaudeUsage") { print(owner, w[kCGWindowLayer as String] ?? "", w[kCGWindowBounds as String] ?? "") } } }'
```
...
- Open the new image with `view_image` and confirm it is ClaudeUsage, not Codex or Script Editor.
...
- Earlier experiments changed language by writing `UserDefaults` and restarting the app. Do not use that for real screenshot passes. The required workflow is to switch language in ClaudeUsage settings itself.
...
- `System Events click at {x, y}` can return a ClaudeUsage AX object while CleanShot still captures the wrong window. CleanShot window mode depends on the real cursor hover target, so move the cursor with CoreGraphics before clicking.
- Always record the latest Downloads file before capture. One earlier attempt found a new file that was not the intended screenshot, and another captured Codex instead of ClaudeUsage.
...
- ClaudeUsage can occasionally become unresponsive when rapidly automated through restart/open cycles. Do not force-kill it repeatedly. If the app stops responding to Computer Use or LaunchServices returns `-1712`, stop the batch, let the user restart the app, then continue with a gentler per-language flow.

Common failure signs:

- A screenshot of Codex means the final click did not use real mouse hover over ClaudeUsage.
