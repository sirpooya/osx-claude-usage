# ClaudeUsage 多限制显示功能实现文档

> 已归档（2026-07-17）：本文档描述的是历史某一时点的实现计划，仅作参考，勿当作现行事实。

## 项目概述

为 ClaudeUsage 应用添加对多种使用限制的支持，包括模型特定限制（Opus/Sonnet）和额外付费额度（Extra Usage）的监控与显示。
...
本文档为 ClaudeUsage v2.0.0 的完整技术规范。请按照以下功能模块逐步实现，每个功能模块可以独立开发和测试。
